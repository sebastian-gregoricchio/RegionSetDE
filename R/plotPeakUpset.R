...
#'
#' @return An UpSet plot built by \code{ComplexHeatmap::UpSet}, drawn when printed, and open to \code{ComplexHeatmap::draw} for the layout options.
#'
...
plotPeakUpset <-
  function(regionSet,
    ...
    combinationMatrix <- ComplexHeatmap::make_comb_mat(membershipMatrix, mode = "distinct")
    combinationSizes <- ComplexHeatmap::comb_size(combinationMatrix)

    # Past a few dozen intersections the plot is unreadable, the largest ones carry the message
    if (length(combinationSizes) > topIntersections) {
      combinationMatrix <- combinationMatrix[order(combinationSizes, decreasing = TRUE)[seq_len(topIntersections)]]
    }
    ...
    upsetPlot <- ComplexHeatmap::UpSet(m = combinationMatrix,
                                       set_order = names(referenceList),
                                       comb_order = order(ComplexHeatmap::comb_size(combinationMatrix), decreasing = TRUE),
                                       comb_col = "grey20",
                                       bg_col = "grey96",
                                       # The numbers stay horizontal above the bars, ComplexHeatmap tilts them by default on a column annotation
                                       top_annotation = ComplexHeatmap::upset_top_annotation(combinationMatrix,
                                                                                             add_numbers = TRUE,
                                                                                             numbers_rot = 0,
                                                                                             numbers_gp = grid::gpar(fontsize = 7),
                                                                                             gp = grid::gpar(fill = "grey30", col = NA),
                                                                                             annotation_name_gp = grid::gpar(fontsize = 9)),
                                       right_annotation = ComplexHeatmap::upset_right_annotation(combinationMatrix,
                                                                                                 add_numbers = TRUE,
                                                                                                 numbers_gp = grid::gpar(fontsize = 7),
